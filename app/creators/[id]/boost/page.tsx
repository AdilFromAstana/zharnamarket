...
export default function CreatorBoostPage() {
  ...
  const currentBoost = CREATOR_BOOST_OPTIONS.find((b) => b.id === selectedBoost)!;
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      const res = await fetch(
        `/api/promo/validate?code=${encodeURIComponent(promoCodeInput.trim())}&type=creator_boost&amount=${currentBoost.price}`,
      );
  ...
  const handlePay = async () => {
    ...
    try {
      const result = await api.post<{
        ...
        status: string;
        isFree?: boolean;
      }>(`/api/payments/creators/${id}/boost`, {
    ...
    <PublicLayout>
      ...
      <div className="max-w-2xl pb-28">
        ...
        <div className="space-y-3 mb-6">
          {CREATOR_BOOST_OPTIONS.map((boost) => (
            <button
              key={boost.id}
        ...
        <Alert
          ...
          description="Продвижение начнёт действовать сразу после подтверждения оплаты. Профиль поднимется в каталоге в течение нескольких минут."
          className="mb-6"
        />
      </div>
      ...
      <StickyBottomBar
        ...
        onBack={() => router.push("/creators/manage")}
      />
    </PublicLayout>
  );
}
