...
export default function CabinetPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const [userData, adsData, profilesData, paymentsData, reviewsData] =
          await Promise.all([
            api.get<UserData>("/api/users/me"),
            api
              .get<Ad[]>("/api/tasks/my?status=active")
              .catch(() => [] as Ad[]),
            api
              .get<unknown[]>("/api/creators/my")
              .then((d) => (d ?? []).map(mapCreatorFromApi))
  ...
  return (
    <>
      ...
      {/* ── Секция 2: Кошелёк ── */}
      <div className="bg-white rounded-2xl border border-gray-200 p-4 md:p-5 mb-6 flex items-center justify-between gap-4">
        <div className="min-w-0">
          <p className="text-xs font-medium text-gray-500 mb-0.5 flex items-center gap-1.5">
                    ...
                    {myAds.length > 0 ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-8">
                        ...
                        <p className="text-gray-400 text-xs text-center max-w-[260px] mb-4">
                          Создайте задание — получите прямые обращения от
                          креаторов
                        </p>
                        <Link href="/ads/new">
                          <Button
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">История ваших оценок</p>
            </div>
          </div>
